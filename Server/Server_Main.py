...
def create_debit_card(customer_id, pin, path):
    ...
    debit_card_key = numpy_random.bytes(64)
    db_interface.set_up_debit_card(customer_id, debit_card_number, debit_card_key)
    aes_e, aes_d = get_aes(hashcode(pin.encode(UTF8STR)))
...
def transfer(transfer_type, transmitter_account_id, receiver_account_id, amount, reference, query_autoincrement_id=False):
    ...
    db_interface.create_transfer(transfer_type, transmitter_account_id, receiver_account_id, amount, reference)
    if query_autoincrement_id:
        autoincrement_id = db_interface.query_last_autoincrement_id()[0]
    else:
        autoincrement_id = None
    db_interface.release_lock()
